...
if __name__ == "__main__":
    ...
    currentProc = 0
    processList = []
    congControlList = select_experiment_protocols(["bbr3", "bbr", "orbtcp", "cubic", "satcp", "leocc"])
    experiment = "experiment5FrontTail"
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 1
        subprocess.Popen("python3 generateExperiment5FrontTailScenarios.py", shell=True).communicate(timeout=30)
        subprocess.Popen("python3 generateExperiment5FrontTailIniFile.py", shell=True).communicate(timeout=30)
        Path("experiment5FrontTailrunTimes.txt").unlink(missing_ok=True)
        
        with open('experiment5FrontTailrunTimes.txt', 'w') as f1:
            f1.write("--Experiment 5 Runtimes (s)--")
            simulation_configs = []
            for cc in congControlList:
                for bs in buffersizes:
                    print("----------queueing experiment 5 " + cc + " " + bs + " simulations------------")
                    iniName = "experiment5FrontTail_" + cc + "_" + bs + ".ini"
                    simulation_configs.extend(collect_simulation_configs(
                        cc, iniName, runList, "../../paperExperiments/experiment5FrontTail"
                    ))
            run_simulation_configs(simulation_configs, "../../paperExperiments/experiment5FrontTail", cores, f1)
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 2
        currentProc = 0
        print("\nAll experiments in Experiment 5 has been run!\n")
        folderLoc =  '../../paperExperiments/experiment5FrontTail/results/'
        print("------------ Generating CSV Files for experiment 5 ------------")
        
        fileList = []
        for file in os.listdir(folderLoc):
            if(file.endswith(".vec")):
                f = os.path.join(folderLoc, file)
                processList.append(subprocess.Popen("opp_scavetool export -o "+ "results/"+ file[:-7] + ".csv -F CSV-R " + "results/" + file , shell=True, cwd='../../paperExperiments/experiment5FrontTail/'))
        ...
        processList.clear()
        currentProc = 0
    print("CSVs created for experiments 5!\n")
    currStep += 1
    
    if(currStep <= endStep and currStep >= startStep): #STEP 3
        currentProc = 0
        print("Extracting CSV data!!\n")
        print("------------ Extracting CSV Files for experiment 5------------")
        processListStr = []
        for protocol in congControlList:
            for buf in buffersizes:
                for rtt in clientsRtts:
                    for run in runList:
                        filePath = '../../paperExperiments/experiment5FrontTail/results/'+ protocol_config_prefix(protocol) + '_' + str(rtt) + 'ms' + '_' + buf + '_Run' + str(run) + '.csv'
                        print(filePath)
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 4
        subprocess.Popen("mkdir ../../plots/experiment5FrontTail", shell=True).communicate(timeout=10) 
        subprocess.Popen("rm -r *", shell=True, cwd='../../plots/experiment5FrontTail').communicate(timeout=200) 
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 5
        ...
        subprocess.Popen("mkdir cumulative", shell=True, cwd='../../plots/experiment5FrontTail/').communicate(timeout=10)
        time.sleep(3)
        p = subprocess.Popen("python3 ../../../pythonScripts/runPlotVariantsFrontTail.py ../../../pythonScripts/experiment5FrontTail/plotGoodputRatio.py", shell=True, cwd='../../plots/experiment5FrontTail/cumulative')
        p.wait(timeout=3600)
        time.sleep(1)
    currStep += 1
            ...
            for buf in buffersizes:
                for rtt in clientsRtts:
                    for run in runList:
                        ...
                        dirPath = '../../plots/experiment5FrontTail/' + protocol + '/' + buf + '/' + str(rtt) + 'ms' + '/run' + str(run) + '/'
                        ...
                        for cwndFile in cwndFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment5FrontTail/plotCwnd.py " + cwndFile[0], dirPath + cwndFile[1]))
                        
                        for rttFile in rttFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment5FrontTail/plotRtt.py " + rttFile[0], dirPath + rttFile[1]))
                                
                        # for tauFile in tauFileList:
                        #     processListStr.append(("python3 ../../../../../../../pythonScripts/experiment5FrontTail//plotTau.py " + tauFile[0], dirPath + tauFile[1]))
                                
                        # for UFile in UFileList:
                        #     processListStr.append(("python3 ../../../../../../../pythonScripts/experiment5FrontTail/plotU.py " + UFile[0], dirPath + UFile[1]))
                                
                        for goodputFile in goodputFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment5FrontTail/plotGoodput.py " + goodputFile[0], dirPath + goodputFile[1]))
                                
                        for queueLengthFile in queueLengthFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment5FrontTail/plotQueueLength.py " + queueLengthFile[0], dirPath + queueLengthFile[1]))
                                
                        for aggrePlotFile in aggrPlotsFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment5FrontTail/plotCwnd.py " + aggrePlotFile[0], dirPath + aggrePlotFile[1]))
                            
                        for aggreGpPlotFile in aggrPlotsGoodputFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment5FrontTail/plotGoodput.py " + aggreGpPlotFile[0], dirPath + aggreGpPlotFile[1]))
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 7
        ...
        merge_pdfs_in_folders("../../plots/experiment5FrontTail")
