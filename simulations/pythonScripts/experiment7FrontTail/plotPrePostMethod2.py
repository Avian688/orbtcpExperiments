    ...
    for mult in QMULTS:
        for rttval in RTTVALS:
                ...
                for delay in DELAYS:
                    for protocol in PROTOCOLS:
                        ...
                        for run in RUNS:
                            ...
                            PATH = '../../../paperExperiments/experiment3FrontTail/csvs/'+ protocol + '/' + QMULTDICT.get(mult) + '/' + str(delay) + 'ms/run' + str(run) + '/'
            ...
            fig_cross.legend(
                ...
                **compact_protocol_legend_kwargs(cross_labels),
            )
            fig_return.legend(
                return_handles,
                return_labels,
                loc='upper center',
                bbox_to_anchor=(0.5, 1.28),
                **compact_protocol_legend_kwargs(return_labels),
            )
            # Save or show figures
            fig_cross.tight_layout()
            fig_cross.savefig(
                f"jains_cross_qmult_{mult}_{rttval}rtts.pdf",
                dpi=1080,
                bbox_inches="tight",
                pad_inches=0.02,
            )
            ...
            fig_return.savefig(
                f"jains_return_qmult_{mult}_{rttval}rtts.pdf",
                dpi=1080,
                bbox_inches="tight",
                pad_inches=0.02,
            )
