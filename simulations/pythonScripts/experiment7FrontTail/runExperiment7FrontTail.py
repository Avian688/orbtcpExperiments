...
from raynetExperimentSupportFrontTail import collect_simulation_configs, protocol_config_prefix, run_simulation_configs, select_experiment_protocols
...
if __name__ == "__main__":
    ...
    currentProc = 0
    processList = []
    congControlList = select_experiment_protocols(["orbtcp", "bbr", "cubic", "bbr3", "satcp", "leocc"])
    experiment = "experiment7FrontTail"
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 1
        subprocess.Popen("python3 generateExperiment7FrontTailScenario.py", shell=True).communicate(timeout=30)
        subprocess.Popen("python3 generateExperiment7FrontTailIniFile.py", shell=True).communicate(timeout=30)
        Path("experiment7FrontTailrunTimes.txt").unlink(missing_ok=True)
        
        with open('experiment7FrontTailrunTimes.txt', 'w') as f1:
            f1.write("--Experiment 7 Runtimes (s)--")
            simulation_configs = []
            for cc in congControlList:
                for bs in buffersizes:
                    print("----------queueing experiment 7 " + cc + " " + bs + " simulations------------")
                    iniName = "experiment7FrontTail_" + cc + "_" + bs + ".ini"
                    simulation_configs.extend(collect_simulation_configs(
                        cc, iniName, runList, "../../paperExperiments/experiment7FrontTail"
                    ))
            run_simulation_configs(simulation_configs, "../../paperExperiments/experiment7FrontTail", cores, f1)
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 2
        currentProc = 0
        print("\nAll experiments in Experiment 7 has been run!\n")
        folderLoc =  '../../paperExperiments/experiment7FrontTail/results/'
        print("------------ Generating CSV Files for experiment 7 ------------")
        
        fileList = []
        for file in os.listdir(folderLoc):
            if(file.endswith(".vec")):
                f = os.path.join(folderLoc, file)
                processList.append(subprocess.Popen("opp_scavetool export -o "+ "results/"+ file[:-7] + ".csv -F CSV-R " + "results/" + file , shell=True, cwd='../../paperExperiments/experiment7FrontTail/'))
                ...
                if(currentProc == cores):
                     ...
                     currentProc = 0
                    #  for fil in fileList:
                    #     subprocess.Popen("rm results/" + fil , shell=True, cwd='../../paperExperiments/experiment7FrontTail/').communicate(timeout=10) #Remove VEC
                    #     subprocess.Popen("rm results/" + fil[:-4] + ".vci" , shell=True, cwd='../../paperExperiments/experiment7FrontTail/').communicate(timeout=10) #Remove VCI
                    #     subprocess.Popen("rm results/" + fil[:-4] + ".sca" , shell=True, cwd='../../paperExperiments/experiment7FrontTail/').communicate(timeout=10) #Remove VCI
                     fileList.clear()
                     processList.clear()
        ...
        print("CSVs created for experiments 7!\n")
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 3
        ...
        print("------------ Extracting CSV Files for experiment 7------------")
        processListStr = []
            ...
            for buf in buffersizes:
                for rtt in rtts:
                    for di in disruptionIntervals:
                        for run in runList:
                            filePath = '../../paperExperiments/experiment7FrontTail/results/'+ protocol_config_prefix(protocol) + "_" + buf + "_RTT" + str(rtt) + 'ms_Disruption' + str(di) + 'ms_Run' + str(run) + '.csv'
                            print(filePath)
                            if os.path.exists(filePath):
                                print("Extracting CSV file for " + experiment + " " + protocol + " " + buf + " " + str(rtt) + " " + str(di) + " " + str(run))
    
                                processListStr.append("python3 extractSingleCsvFile.py " + filePath + " " + experiment + " " + protocol + " " + buf + " " + str(rtt) + " " + str(di) + " " + str(run))
        time.sleep(10)
        currentProc = 0
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 4
        subprocess.Popen("mkdir ../../plots/experiment7FrontTail", shell=True).communicate(timeout=10) 
        subprocess.Popen("rm -r *", shell=True, cwd='../../plots/experiment7FrontTail').communicate(timeout=200) 
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 5
        ...
        subprocess.Popen("mkdir cumulative", shell=True, cwd='../../plots/experiment7FrontTail/').communicate(timeout=10)
        time.sleep(3)
        p = subprocess.Popen("python3 ../../../pythonScripts/runPlotVariantsFrontTail.py ../../../pythonScripts/experiment7FrontTail/plotAggrGoodputInterval.py", shell=True, cwd='../../plots/experiment7FrontTail/cumulative')
        p.wait(timeout=3600)
        time.sleep(1)
    currStep += 1
                ...
                for rtt in rtts:
                    for di in disruptionIntervals:
                        for run in runList:
                            ...
                            dirPath = '../../plots/experiment7FrontTail/' + protocol + '/' + buf + '/' + str(rtt) + 'ms/Disruption' + str(di) +'ms/run' + str(run) + '/'
                            ...
                            for cwndFile in cwndFileList:
                                processListStr.append(("python3 ../../../../../../../../pythonScripts/experiment7FrontTail/plotCwnd.py " + cwndFile[0], dirPath + cwndFile[1]))
                            
                            for rttFile in rttFileList:
                                processListStr.append(("python3 ../../../../../../../../pythonScripts/experiment7FrontTail/plotRtt.py " + rttFile[0], dirPath + rttFile[1]))
                                    
                            for UFile in UFileList:
                                processListStr.append(("python3 ../../../../../../../../pythonScripts/experiment7FrontTail/plotU.py " + UFile[0], dirPath + UFile[1]))
                                    
                            for goodputFile in goodputFileList:
                                processListStr.append(("python3 ../../../../../../../../pythonScripts/experiment7FrontTail/plotGoodput.py " + goodputFile[0], dirPath + goodputFile[1]))
                                    
                            for queueLengthFile in queueLengthFileList:
                                processListStr.append(("python3 ../../../../../../../../pythonScripts/experiment7FrontTail/plotQueueLength.py " + queueLengthFile[0], dirPath + queueLengthFile[1]))
                                    
                            for aggrePlotFile in aggrPlotsFileList:
                                processListStr.append(("python3 ../../../../../../../../pythonScripts/experiment7FrontTail/plotCwnd.py " + aggrePlotFile[0], dirPath + aggrePlotFile[1]))
                                
                            for aggreGpPlotFile in aggrPlotsGoodputFileList:
                                processListStr.append(("python3 ../../../../../../../../pythonScripts/experiment7FrontTail/plotGoodput.py " + aggreGpPlotFile[0], dirPath + aggreGpPlotFile[1]))
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 7
        ...
        merge_pdfs_in_folders("../../plots/experiment7FrontTail")
