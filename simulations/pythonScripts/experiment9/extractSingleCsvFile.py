    ...
    for vec in vectorsToExtract:    
        ...
        for mod in range(len(results.vecvalue.to_numpy())):
            if(not results.vecvalue.to_numpy()[mod] is None):
                ...
                subprocess.Popen("mkdir -p ../../paperExperiments/" + exp + "/csvs/" + protocol + '/' + sourceDestination, shell=True).communicate(timeout=40) 
                subprocess.Popen("mkdir -p ../../paperExperiments/" + exp + "/csvs/" + protocol + '/' + sourceDestination + '/' + mode, shell=True).communicate(timeout=40) 
                subprocess.Popen("mkdir -p ../../paperExperiments/" + exp + "/csvs/" + protocol + '/' + sourceDestination + '/' + mode + '/' + bufferName, shell=True).communicate(timeout=40) 
                subprocess.Popen("mkdir -p ../../paperExperiments/" + exp + "/csvs/" + protocol + '/' + sourceDestination + '/' + mode + '/' + bufferName + '/run'+ str(run), shell=True).communicate(timeout=40)
                subprocess.Popen("mkdir -p ../../paperExperiments/" + exp + "/csvs/" + protocol + '/' + sourceDestination + '/' + mode + '/' + bufferName + '/run'+ str(run) + "/" + str(modName), shell=True).communicate(timeout=40)
                finallist.to_csv('../../paperExperiments/'+ exp +'/csvs/' + protocol + '/' + sourceDestination + '/'+ mode + '/' + bufferName + '/run'+ str(run) + '/' + str(modName) + '/' + vec + '.csv', index=False)
                extracted = True
    termTime.sleep(1)            
