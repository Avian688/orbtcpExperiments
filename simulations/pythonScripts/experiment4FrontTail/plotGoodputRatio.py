...
for mult in QMULTS:
    ...
    for protocol in PROTOCOLS:
        for bw in BWS:
            for delay in DELAYS:
                ...
                for run in RUNS:
                    PATH = '../../../paperExperiments/experiment4FrontTail/csvs/' + protocol + '/' + QMULTDICT.get(mult) + '/' + str(delay) + 'ms/run' + str(run) + '/'
    ...
    export_plot_dataframe(
        f"goodput_ratio_intra_rtt_{mult}_points.csv",
        ...
        metadata={
            ...
            "plot": f"goodput_ratio_intra_rtt_{mult}",
