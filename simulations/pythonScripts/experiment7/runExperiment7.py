...
if __name__ == "__main__":
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 1
        ...
        with open('experiment7runTimes.txt', 'w') as f1:
            ...
            run_simulation_configs(simulation_configs, "../../paperExperiments/experiment7", cores, f1)
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 2
        ...
        folderLoc =  '../../paperExperiments/experiment7/results/'
        ...
        for file in os.listdir(folderLoc):
            if(file.endswith(".vec")):
                ...
                processList.append(subprocess.Popen("opp_scavetool export -o "+ "results/"+ file[:-7] + ".csv -F CSV-R " + "results/" + file , shell=True, cwd='../../paperExperiments/experiment7/'))
                ...
                if(currentProc == cores):
                     ...
                     currentProc = 0
                    #  for fil in fileList:
                    #     subprocess.Popen("rm results/" + fil , shell=True, cwd='../../paperExperiments/experiment7/').communicate(timeout=10) #Remove VEC
                    #     subprocess.Popen("rm results/" + fil[:-4] + ".vci" , shell=True, cwd='../../paperExperiments/experiment7/').communicate(timeout=10) #Remove VCI
                    #     subprocess.Popen("rm results/" + fil[:-4] + ".sca" , shell=True, cwd='../../paperExperiments/experiment7/').communicate(timeout=10) #Remove VCI
                     fileList.clear()
                     processList.clear()
            ...
            for buf in buffersizes:
                for rtt in rtts:
                    for di in disruptionIntervals:
                        for run in runList:
                            filePath = '../../paperExperiments/experiment7/results/'+ protocol_config_prefix(protocol) + "_" + buf + "_RTT" + str(rtt) + 'ms_Disruption' + str(di) + 'ms_Run' + str(run) + '.csv'
                            print(filePath)
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 4
        subprocess.Popen("mkdir ../../plots/experiment7", shell=True).communicate(timeout=10) 
        subprocess.Popen("rm -r *", shell=True, cwd='../../plots/experiment7').communicate(timeout=200) 
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 5
        ...
        time.sleep(3)
        p = subprocess.Popen("python3 ../../../pythonScripts/runPlotVariants.py ../../../pythonScripts/experiment7/plotAggrGoodputInterval.py", shell=True, cwd='../../plots/experiment7/cumulative')
        p.wait(timeout=3600)
        time.sleep(1)
    currStep += 1
