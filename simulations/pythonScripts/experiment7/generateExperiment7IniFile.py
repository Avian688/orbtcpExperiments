...
if __name__ == "__main__":
    ...
    for alg in algorithms:
        for qs in queueSizes:
            ...
            fileName =  '../../paperExperiments/experiment7/experiment7_' + alg + "_" + queueIniTitle +'.ini'
            ...
            with open(fileName, 'w') as f:
                ...
                dir = [f for f in os.listdir('../../paperExperiments/scenarios/experiment7/.')]
                for xmlFile in dir:
                    runName = str(os.path.basename(xmlFile)[0:-4])
                    for i in range(numOfRuns):
                        random.seed(simSeed + i)
                        rtt = int(re.search(r'RTT(\d+)ms', runName).group(1))
                        configName = alg.title() + '_' + queueIniTitle + '_' + str(runName)
                        clientStart = random.uniform(0,0.5)
                        f.write('\n' + '[Config ' + configName + '_Run' + str(i+1) + ']')
                        f.write('\n' + 'extends = General \n')
                        f.write('\n' + '**.numberOfFlows = 1 \n')
                        f.write('\n' + '*.client[0].app[0].connectAddress = "server[0]"')
                        f.write('\n' + '*.client[0].app[0].tOpen  = '+ str(clientStart) +'s')
                        f.write('\n' + '*.client[0].app[0].tSend = '+ str(clientStart) +'s\n')
                        f.write('\n' + '**.ppp[*].queue.packetCapacity = ' + str(math.ceil((((rtt/1000)*bandwidth)/1448)*qs)) + '\n')
                        f.write('\n' + '*.scenarioManager.script = xmldoc("../scenarios/experiment7/' + str(runName) + '.xml")\n')
    print('\nINI files generated!')
