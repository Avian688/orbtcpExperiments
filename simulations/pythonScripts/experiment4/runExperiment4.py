...
if __name__ == "__main__":
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 1
        ...
        Path("experiment4runTimes.txt").unlink(missing_ok=True)
        ...
        for protocol in congControlList:
            for buf in buffersizes:
                for rtt in clientsRtts:
                    for run in runList:
                        filePath = '../../paperExperiments/experiment4/results/'+ protocol_config_prefix(protocol) + '_' + str(rtt) + 'ms' + '_' + buf + '_Run' + str(run) + '.csv'
                        print(filePath)
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 5
        ...
        time.sleep(3)
        p = subprocess.Popen("python3 ../../../pythonScripts/runPlotVariants.py ../../../pythonScripts/experiment4/plotGoodputRatio.py", shell=True, cwd='../../plots/experiment4/cumulative')
        p.wait(timeout=3600)
        time.sleep(1)
    currStep += 1
