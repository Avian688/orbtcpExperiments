    ...
    for vec in vectorsToExtract:    
        ...
        for mod in range(len(results.vecvalue.to_numpy())):
            if(not results.vecvalue.to_numpy()[mod] is None):
                ...
                subprocess.Popen("mkdir -p ../../paperExperiments/" + exp + "/csvs/ping", shell=True).communicate(timeout=40)
                subprocess.Popen("mkdir -p ../../paperExperiments/" + exp + "/csvs/ping/" + mode, shell=True).communicate(timeout=40) 
                subprocess.Popen("mkdir -p ../../paperExperiments/" + exp + "/csvs/ping/" + mode + "/" + str(modName), shell=True).communicate(timeout=40)
                finallist.to_csv('../../paperExperiments/'+ exp +"/csvs/ping/" + mode + "/" + str(modName) + '/' + vec + '.csv', index=False)
                extracted = True
    termTime.sleep(1)            
