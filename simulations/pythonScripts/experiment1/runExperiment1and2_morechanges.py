...
# Steps:
    # 1: Generate and run exp1
    # 2: Generate and run exp2
    # 3: Generate CSVs (summary) for exp1
    # 4: Generate CSVs (summary) for exp2
    # 5: Extract CSVs for exp1/exp2
    # 6-9: Plotting      
if __name__ == "__main__":
    ...
    currentProc = 0
    processList = []
    raynetCongControlList = ["orca", "cleanslate", "astrea"] # RayNet; List of RL schemes available 
    congControlList = ["orca"]
            ...
            for cc in congControlList:
                ...
                for line in iniFile:
                    if line.find('[Config') != -1:
                        configName = (line[8:])[:-2]
                        progStart = time.time()
                        if cc in raynetCongControlList: # RayNet; Use RayNet runner instead of opp_run for RL schemes
                            processList.append(subprocess.Popen(
                                os.getenv('HOME') + "/raynet/.venv/bin/python ~/raynet/_scripts/run/raynet_runner.py " +
                                "orca "                          # Protocol
                                "experiment1_" + cc + ".ini " +  # .ini
                                configName,                     # Section
                                shell=True, cwd='../../paperExperiments/experiment1', stdout=None, stderr=None))
                        else:
                            processList.append(subprocess.Popen(
                                ...
                                "../../../../leosatellites/simulations:../../../../leosatellites/src:"
                                "../../../../os3/simulations:../../../../os3/src:"  
                                "/home/james/raynet/simlibs/RLComponents/src" # RayNet; RLComponents must be included as an NED source to provide access to Broker
                                ...
                                " -l /home/james/raynet/simlibs/RLComponents/src/RLComponents" # RayNet; RLComponents must be linked to provide access to Broker
                                " experiment1_" + cc + ".ini",
                                shell=True, cwd='../../paperExperiments/experiment1', stdout=None, stderr=None))
                ...
                for line in iniFile:
                    if line.find('[Config') != -1:
                        ...
                        processList.append(subprocess.Popen(
                            ...
                            " -l ../../../../tcpGoodputApplications/src/tcpGoodputApplications"
                            " experiment2_" + cc + ".ini",
                            shell=True, cwd='../../paperExperiments/experiment2', stdout=None, stderr=None))
                        currentProc = currentProc + 1
                        print("Running simulation [" + configName + "]... (Run #" + str(currentProc) + ")")
                        if(currentProc == cores):
                            procCompleteNum = 0
    ...
    if(currStep <= endStep and currStep >= startStep and "experiment1" in experiments): #STEP 3
        currentProc = 0
        ...
        for file in os.listdir(folderLoc):
            if(file.endswith(".vec")):
                ...
                processList.append(subprocess.Popen("opp_scavetool i "+ "results/"+ file, shell=True, cwd='../../paperExperiments/experiment1/'))
    ...
    if(currStep <= endStep and currStep >= startStep and "experiment2" in experiments): #STEP 4
            ...
            for protocol in congControlList:
                for run in runList:
                    ...
                    if os.path.exists(cwndFilePath) or os.path.exists(goodputFilePath) or os.path.exists(throughputFilePath) or os.path.exists(queueLengthFilePath):
                        #subprocess.Popen("mkdir goodput", shell=True, cwd='plots/'+ exp + '/' + protocol +'/run' + str(run) + '/', stdout=None, stderr=subprocess.STDOUT).communicate(timeout=10)
                        #subprocess.Popen("mkdir throughput", shell=True, cwd='plots/'+ exp + '/' + protocol +'/run' + str(run) + '/', stdout=None, stderr=subprocess.STDOUT).communicate(timeout=10) 
                        #subprocess.Popen("mkdir cwnd", shell=True, cwd='plots/'+ exp + '/' + protocol +'/run' + str(run) + '/', stdout=None, stderr=subprocess.STDOUT).communicate(timeout=10) 
                        #subprocess.Popen("mkdir queueLength", shell=True, cwd='plots/'+ exp + '/' + protocol +'/run' + str(run) + '/', stdout=None, stderr=subprocess.STDOUT).communicate(timeout=10) 
                        #subprocess.Popen("mkdir rtt", shell=True, cwd='plots/'+ exp + '/' + protocol +'/run' + str(run) + '/', stdout=None, stderr=subprocess.STDOUT).communicate(timeout=10) 
