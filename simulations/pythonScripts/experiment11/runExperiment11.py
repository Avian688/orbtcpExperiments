...
if __name__ == "__main__":
        ...
        for protocol in congControlList:
            for flowBatch in flowBatches:
                for rtt in clientsRtts:
                    for run in runList:
                        filePath = '../../paperExperiments/experiment11/results/'+ protocol_config_prefix(protocol) + '_' + str(flowBatch) + 'flows_' + str(rtt) + 'ms_Run' + str(run) + '.csv'
                        print(filePath)
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 5
        ...
        time.sleep(3)
        p = subprocess.Popen("python3 ../../../pythonScripts/runPlotVariants.py ../../../pythonScripts/experiment11/plotScatterUtilDelay.py", shell=True, cwd='../../plots/experiment11/cumulative')
        p.wait(timeout=3600)
        time.sleep(1)
    currStep += 1
