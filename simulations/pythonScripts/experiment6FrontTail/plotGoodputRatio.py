...
from plotDataExportFrontTail import export_plot_dataframe
from plotProtocolSupportFrontTail import (
    LEO_PROTOCOLS,
    PROTOCOL_COLORS,
    PROTOCOL_LABELS,
    PROTOCOL_MARKERS,
...
for mult in QMULTS:
    ...
    for protocol in PROTOCOLS:
        for bw in BWS:
            for delay in DELAYS:
                ...
                for run in RUNS:
                    PATH = '../../../paperExperiments/experiment6FrontTail/csvs/' + protocol + '/' + QMULTDICT.get(mult) + '/' + str(delay) + 'ms/run' + str(run) + '/'
                    ...
                    if all(os.path.exists(f) for f in receiver_files_ribs) and os.path.exists(receiver_file_spine):
                        ...
                        combined_ribs = pd.concat(
                            [rib['goodput'] for rib in receivers_ribs],
                            axis=1,
                            keys=[f'Rib{i+1}' for i in range(len(receivers_ribs))]
                        )
                        max_bandwidth_between_ribs = combined_ribs.max(axis=1).reset_index()
                        max_bandwidth_between_ribs.columns = ['time', 'goodput']
                        max_bandwidth_between_ribs = max_bandwidth_between_ribs.set_index('time')

                        total = receiver_spine[['goodput']].join(
                            max_bandwidth_between_ribs,
                            how='inner',
                            lsuffix='1',
                            rsuffix='2'
                        )
                ...
                if len(goodput_ratios_total) > 0:
                    ...
                    if len(goodput_ratios_total) > 0:
                        data_entry = [
                            ...
                            goodput_ratios_total.mean(),
                            goodput_ratios_total.std()
                        ]
    ...
    summary_data = pd.DataFrame(
        ...
        columns=[
            ...
            'qmult', 'goodput_ratio_total_mean',
            'goodput_ratio_total_std'
        ]
    )
    export_plot_dataframe(
        f"goodput_ratio_between_max_ribs_magic_goodput_q{mult}_points.csv",
        ...
        metadata={
            ...
            "plot": f"goodput_ratio_between_max_ribs_magic_goodput_q{mult}",
            "description": "Final error-bar points plotted for parking-lot goodput ratio versus RTT.",
        },
    )
    ...
    for y, label, offset, color in [
        (1, 'max-min', -0.11, "red"),
        (1/FLOWS / ((FLOWS-1)/FLOWS), 'proportional', 0.02, "black")
    ]:
    ...
    for fmt in ['pdf']:
        plt.savefig(
            f'goodput_ratio_between_max_ribs_magic_goodput_q{mult}.{fmt}',
            dpi=1080,
            bbox_inches="tight",
            pad_inches=0.02,
        )
