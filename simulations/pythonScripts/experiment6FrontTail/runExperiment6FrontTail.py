...
if __name__ == "__main__":
    ...
    currentProc = 0
    processList = []
    congControlList = select_experiment_protocols(["bbr", "orbtcp", "cubic", "bbr3", "satcp", "leocc"])
    experiment = "experiment6FrontTail"
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 1
        subprocess.Popen("python3 generateExperiment6FrontTailScenarios.py", shell=True).communicate(timeout=30)
        subprocess.Popen("python3 generateExperiment6FrontTailIniFile.py", shell=True).communicate(timeout=30)
        Path("experiment6FrontTailrunTimes.txt").unlink(missing_ok=True)
        
        with open('experiment6FrontTailrunTimes.txt', 'w') as f1:
            f1.write("--Experiment 6 Runtimes (s)--")
            simulation_configs = []
            for cc in congControlList:
                for bs in buffersizes:
                    print("----------queueing experiment 6 " + cc + " " + bs + " simulations------------")
                    iniName = "experiment6FrontTail_" + cc + "_" + bs + ".ini"
                    simulation_configs.extend(collect_simulation_configs(
                        cc, iniName, runList, "../../paperExperiments/experiment6FrontTail"
                    ))
            run_simulation_configs(simulation_configs, "../../paperExperiments/experiment6FrontTail", cores, f1)
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 2
        ...
        folderLoc =  '../../paperExperiments/experiment6FrontTail/results/'
        ...
        for file in os.listdir(folderLoc):
            if(file.endswith(".vec")):
                ...
                processList.append(subprocess.Popen("opp_scavetool export -o "+ "results/"+ file[:-7] + ".csv -F CSV-R " + "results/" + file , shell=True, cwd='../../paperExperiments/experiment6FrontTail/'))
                ...
                if(currentProc == cores):
                     ...
                     currentProc = 0
                         #for fil in fileList:
                         #   subprocess.Popen("rm results/" + fil , shell=True, #cwd='../../paperExperiments/experiment3FrontTail/') #Remove VEC
                         #   subprocess.Popen("rm results/" + fil[:-4] + ".vci" , shell=True, #cwd='../../paperExperiments/experiment3FrontTail/') #Remove VCI
                        #    subprocess.Popen("rm results/" + fil[:-4] + ".sca" , shell=True, #cwd='../../paperExperiments/experiment3FrontTail/') #Remove VCI
                     fileList.clear()
                     processList.clear()
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 3
        ...
        for protocol in congControlList:
            for buf in buffersizes:
                for rtt in clientsRtts:
                    for run in runList:
                        filePath = '../../paperExperiments/experiment6FrontTail/results/'+ protocol_config_prefix(protocol) + "_" + str(rtt) + 'ms' + "_" + buf + "_" + 'Run' + str(run) + '.csv'
                        print(filePath)
        ...
        #subprocess.Popen("rm  ../../paperExperiments/experiment6FrontTail/results/*.vec", shell=True).communicate(timeout=60)
        ##subprocess.Popen("rm  ../../paperExperiments/experiment6FrontTail/results/*.vci", shell=True).communicate(timeout=60)
        #subprocess.Popen("rm  ../../paperExperiments/experiment6FrontTail/results/*.sca", shell=True).communicate(timeout=60)     

    currStep += 1
    
    if(currStep <= endStep and currStep >= startStep): #STEP 4
        subprocess.Popen("mkdir ../../plots/experiment6FrontTail", shell=True).communicate(timeout=10) 
        subprocess.Popen("rm -r *", shell=True, cwd='../../plots/experiment6FrontTail').communicate(timeout=200) 
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 5
        ...
        subprocess.Popen("mkdir cumulative", shell=True, cwd='../../plots/experiment6FrontTail/').communicate(timeout=10)
        time.sleep(3)
        p = subprocess.Popen("python3 ../../../pythonScripts/runPlotVariantsFrontTail.py ../../../pythonScripts/experiment6FrontTail/plotGoodputRatio.py", shell=True, cwd='../../plots/experiment6FrontTail/cumulative')
        p.wait(timeout=3600)
        time.sleep(1)
    currStep += 1
            ...
            for buf in buffersizes:
                for rtt in clientsRtts:
                    for run in runList:
                        ...
                        dirPath = '../../plots/experiment6FrontTail/' + protocol + '/' + buf + '/' + str(rtt) + 'ms' + '/run' + str(run) + '/'
                    ...
                    #aggrPlotsGoodputFileList.append((fileStart + '/parkinglot.server[0].app[0]/goodput.csv '+ fileStart +'/parkinglot.server[1].app[0]/goodput.csv '+ fileStart +'/parkinglot.server[1].app[0]/goodput.csv '+ fileStart +'/parkinglot.server[1].app[0]/goodput.csv', "aggPlots"))
                        ...
                        for cwndFile in cwndFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment6FrontTail/plotCwnd.py " + cwndFile[0], dirPath + cwndFile[1]))
                        
                        for rttFile in rttFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment6FrontTail/plotRtt.py " + rttFile[0], dirPath + rttFile[1]))
                                
                        # for tauFile in tauFileList:
                        #     processListStr.append(("python3 ../../../../../../../pythonScripts/experiment6FrontTail//plotTau.py " + tauFile[0], dirPath + tauFile[1]))
                                
                        # for UFile in UFileList:
                        #     processListStr.append(("python3 ../../../../../../../pythonScripts/experiment6FrontTail/plotU.py " + UFile[0], dirPath + UFile[1]))
                                
                        for goodputFile in goodputFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment6FrontTail/plotGoodput.py " + goodputFile[0], dirPath + goodputFile[1]))
                                
                        for queueLengthFile in queueLengthFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment6FrontTail/plotQueueLength.py " + queueLengthFile[0], dirPath + queueLengthFile[1]))
                                
                        for aggrePlotFile in aggrPlotsFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment6FrontTail/plotCwnd.py " + aggrePlotFile[0], dirPath + aggrePlotFile[1]))
                            
                        for aggreGpPlotFile in aggrPlotsGoodputFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment6FrontTail/plotGoodput.py " + aggreGpPlotFile[0], dirPath + aggreGpPlotFile[1]))
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 7
        ...
        merge_pdfs_in_folders("../../plots/experiment6FrontTail")
