...
from raynetExperimentSupportFrontTail import ensure_local_ini_preamble
...
def _validate_transformed_ini(source: Path, source_text: str, pint_text: str) -> None:
    ...
    stale_names = ("OrbtcpFlavour", "IntFrontTailQueue", "IntFrontTailInterface")
    ...
    for section in re.split(r"(?m)(?=^\[(?:General|Config )[^\n]*\]$)", pint_text):
        pint_queue = section.find('queue.typename = "PintFrontTailQueue"')
        fallback = section.find('**.**.queue.typename = "DropHeadQueue"')
        if pint_queue != -1 and fallback != -1 and pint_queue > fallback:
            raise ValueError(
                f"{source} selects PintFrontTailQueue after the DropHeadQueue fallback"
            )
...
def _transform_ini(source: Path) -> str:
    ...
    if '"IntFrontTailQueue"' not in source_text:
        raise ValueError(f"{source} does not select an IntFrontTailQueue")
    ...
    text = text.replace("IntFrontTailQueue", "PintFrontTailQueue")
    text = text.replace("IntFrontTailInterface", "PintFrontTailInterface")
    ...
    source_header = (
        f"# Generated from {source.name} by orbtcpPintExperimentSupportFrontTail.py.\n"
...
def main() -> None:
    ...
    generated = []
    for experiment_dir in sorted(_paper_experiments_root().glob("experiment[0-9]*FrontTail")):
