...
def main() -> None:
    ...
    out_dir = Path("../../paperExperiments/experiment2FrontTail")
    scenarios_dir = Path("../../paperExperiments/scenarios/experiment2FrontTail")
    ...
    for alg in algorithms:
        ...
        fileName = out_dir / f"experiment2FrontTail_{alg}.ini"
        ...
        with fileName.open("w", encoding="utf-8") as f:
            ...
            if alg in ("cubic", "satcp"):
                ...
                w('**.ppp[*].queue.typename = "BandwidthRecorderFrontTailQueue"')
                w()
                # keep your experiment2FrontTail BBR value (you had 500*1448 for bbr, 4000*1448 for bbr3)
                if alg == "bbr":
            ...
            for xmlFile in scenario_files:
                ...
                scenario_doc = f'xmldoc("../scenarios/experiment2FrontTail/run{runNum}.xml")'
                w(f"*.scenarioManager.script = {scenario_doc}")
                if alg == "satcp":
                    w(f"**.tcp.scenario = {scenario_doc}")
                w()

    clone_raynet_ini_variants(out_dir / "experiment2FrontTail_bbr.ini")
