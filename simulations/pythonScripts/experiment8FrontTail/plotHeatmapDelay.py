...
from plotDataExportFrontTail import export_heatmap
...
def get_base_rtt_avg(path_key_after, terminal_index, app_index):
    ping_path = os.path.join(
        "../../../paperExperiments/experiment8FrontTail/csvs/ping",
        path_key_after,
...
def compute_mean_std_normalised(path_key, proto, m):
    ...
    for run in RUNS:
        srtt_path = os.path.join(
            "../../../paperExperiments/experiment8FrontTail/csvs", protocol_config_prefix(proto),
            path_key_before,
            path_key_after,
            QMULTDICT.get(m),
            f"run{run}",
...
for m in QMULTS:
    ...
    df_std[m] = dfs
    export_heatmap(
        f"heatmap_leo_delay_q{m}_points.csv",
        dfm,
        dfs,
        metadata={
            "experiment": "experiment8FrontTail",
            "plot": "heatmap_leo_delay",
            "qmult": m,
