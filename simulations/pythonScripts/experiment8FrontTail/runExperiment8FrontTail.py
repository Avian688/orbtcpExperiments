...
from raynetExperimentSupportFrontTail import SimulationConfig, protocol_config_prefix, run_simulation_configs, select_experiment_protocols
...
def collect_config_entries(paperExperimentDir, congControlList, runList):
    ...
    for cc in congControlList:
        fileName = paperExperimentDir / ("experiment8FrontTail_" + cc + ".ini")
...
def run_config_batch(configEntries, simulationCores, paperExperimentDir):
    configs = [
        SimulationConfig(cc, "experiment8FrontTail_" + cc + ".ini", configName, include_leo=True)
...
if __name__ == "__main__":
    ...
    experiment = "experiment8FrontTail"
    buffersizes = ["mediumbuffer"]#["mediumbuffer", "smallbuffer", "largebuffer"]
    runs = 5
    runList = list(range(1,runs+1))
    scriptDir = Path(__file__).resolve().parent
    paperExperimentDir = (scriptDir / "../../paperExperiments/experiment8FrontTail").resolve()
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 1
        subprocess.Popen("python3 generateExperiment8FrontTailIniFile.py", shell=True).communicate(timeout=30)
        resultsDir = paperExperimentDir / "results"
        os.makedirs(resultsDir, exist_ok=True)
    
        subprocess.Popen("rm experiment8FrontTailrunTimes.txt", shell=True).communicate(timeout=30)
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 2
        currentProc = 0
        print("\nAll experiments in experiment 8 has been run!\n")
        folderLoc =  '../../paperExperiments/experiment8FrontTail/results/'
        print("------------ Generating CSV Files for experiment 8 ------------")
        
        fileList = []
        for file in os.listdir(folderLoc):
            if(file.endswith(".vec")):
                f = os.path.join(folderLoc, file)
                processList.append(subprocess.Popen("opp_scavetool export -o "+ "results/"+ file[:-7] + ".csv -F CSV-R " + "results/" + file , shell=True, cwd='../../paperExperiments/experiment8FrontTail/'))
                ...
                if(currentProc == cores):
                     ...
                     currentProc = 0
                    #  for fil in fileList:
                    #     subprocess.Popen("rm results/" + fil , shell=True, cwd='../../paperExperiments/experiment8FrontTail/').communicate(timeout=10) #Remove VEC
                    #     subprocess.Popen("rm results/" + fil[:-4] + ".vci" , shell=True, cwd='../../paperExperiments/experiment8FrontTail/').communicate(timeout=10) #Remove VCI
                    #     subprocess.Popen("rm results/" + fil[:-4] + ".sca" , shell=True, cwd='../../paperExperiments/experiment8FrontTail/').communicate(timeout=10) #Remove VCI
                     fileList.clear()
                     processList.clear()
        ...
        print("CSVs created for experiment 8!\n")
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 3
        ...
        print("------------ Extracting CSV Files for experiment 8 ------------")
        processListStr = []
                ...
                for city1, city2, modes in city_pairs:
                    ...
                    for mode in modes:
                        for run in runList:
                            ...
                            filePath = '../../paperExperiments/experiment8FrontTail/results/'+ protocolTitle + '_' + city1NoSpace + "To" + city2NoSpace + "_" + mode + '_' + buf +'_Run' + str(run) + '.csv'
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 4
        ...
        processListStr = []
        filePath = '../../paperExperiments/experiment8FrontTail/results/pingIsl.csv'
        if os.path.exists(filePath):
        ...
        filePath2 = '../../paperExperiments/experiment8FrontTail/results/pingBentPipe.csv'
    ...
    #     subprocess.Popen("mkdir ../../plots/experiment8FrontTail", shell=True).communicate(timeout=10) 
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 6
        ...
        subprocess.Popen("mkdir cumulative", shell=True, cwd='../../plots/experiment8FrontTail/').communicate(timeout=10)
        time.sleep(3)
        p = subprocess.Popen("python3 ../../../pythonScripts/runPlotVariantsFrontTail.py ../../../pythonScripts/experiment8FrontTail/plotHeatmapGoodput.py", shell=True, cwd='../../plots/experiment8FrontTail/cumulative')
        p.wait(timeout=3600)
        time.sleep(1)
    currStep += 1
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 7
        ...
        subprocess.Popen("mkdir cumulative", shell=True, cwd='../../plots/experiment8FrontTail/').communicate(timeout=10)
        time.sleep(3)
        p = subprocess.Popen("python3 ../../../pythonScripts/runPlotVariantsFrontTail.py ../../../pythonScripts/experiment8FrontTail/plotHeatmapDelay.py", shell=True, cwd='../../plots/experiment8FrontTail/cumulative')
        p.wait(timeout=3600)
        time.sleep(1)
    currStep += 1
    ...
    #                         dirPath = '../../plots/experiment8FrontTail/' + protocol + '/' + buf + '/' + str(rtt) + 'ms/DI' + str(di) +'ms/run' + str(run) + '/'
    ...
    #                             processListStr.append(("python3 ../../../../../../../pythonScripts/experiment8FrontTail/plotCwnd.py " + cwndFile[0], dirPath + cwndFile[1]))
                            
    #                         for rttFile in rttFileList:
    #                             processListStr.append(("python3 ../../../../../../../pythonScripts/experiment8FrontTail/plotRtt.py " + rttFile[0], dirPath + rttFile[1]))
                                    
    #                         for UFile in UFileList:
    #                             processListStr.append(("python3 ../../../../../../../pythonScripts/experiment8FrontTail/plotU.py " + UFile[0], dirPath + UFile[1]))
                                    
    #                         for goodputFile in goodputFileList:
    #                             processListStr.append(("python3 ../../../../../../../pythonScripts/experiment8FrontTail/plotGoodput.py " + goodputFile[0], dirPath + goodputFile[1]))
                                    
    #                         for queueLengthFile in queueLengthFileList:
    #                             processListStr.append(("python3 ../../../../../../../pythonScripts/experiment8FrontTail/plotQueueLength.py " + queueLengthFile[0], dirPath + queueLengthFile[1]))
                                    
    #                         for aggrePlotFile in aggrPlotsFileList:
    #                             processListStr.append(("python3 ../../../../../../../pythonScripts/experiment8FrontTail/plotCwnd.py " + aggrePlotFile[0], dirPath + aggrePlotFile[1]))
                                
    #                         for aggreGpPlotFile in aggrPlotsGoodputFileList:
    #                             processListStr.append(("python3 ../../../../../../../pythonScripts/experiment8FrontTail/plotGoodput.py " + aggreGpPlotFile[0], dirPath + aggreGpPlotFile[1]))
    ...
    #     merge_pdfs_in_folders("../../plots/experiment8FrontTail")
