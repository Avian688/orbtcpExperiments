...
def getResults(file):
    ...
    vec = vectors[vectors.name == 'additiveIncrease:vector']
    return vec;
...
if __name__ == "__main__":
    ...
    for result in results:
        ...
        plt.ylabel('AI Value (bytes)', fontsize=28)
        plt.legend(loc = "upper right")
        plt.title("Additive Increase", fontsize=35)
        #plt.xticks((np.arange(0, result["Goodput"].idxmax(), step=250)))
        plt.savefig('additiveIncrease.png')
        i += 1
