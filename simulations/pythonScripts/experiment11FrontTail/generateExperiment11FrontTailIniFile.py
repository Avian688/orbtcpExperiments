...
# Uses scenario XMLs in ../../paperExperiments/scenarios/experiment11FrontTail/{5flows,10flows,20flows}/
...
from raynetExperimentSupportFrontTail import clone_raynet_ini_variants, common_ned_path
...
def main() -> None:
    ...
    out_dir = Path("../../paperExperiments/experiment11FrontTail")
    scenarios_root = Path("../../paperExperiments/scenarios/experiment11FrontTail")
    ...
    for alg in algorithms:
        ...
        fileName = out_dir / f"experiment11FrontTail_{alg}.ini"
        ...
        with fileName.open("w", encoding="utf-8") as f:
            ...
            if alg in ("cubic", "satcp"):
                w('**.ppp[*].queue.typename = "BandwidthRecorderFrontTailQueue"')
                w()
                ...
                w('**.**.queue.typename = "DropHeadQueue"')
                w()
            ...
            for numFlows in batchSizes:
                ...
                for runIdx in range(numOfRuns):
                    ...
                    # ORBTCP IntFrontTailQueue must be on router1's bottleneck interface: ppp[numFlows]
                    if alg == "orbtcp":
                        w(f'**.router1.ppp[{numFlows}].queue.typename = "IntFrontTailQueue"')
                        w()
                    ...
                    scenario_doc = f'xmldoc("../scenarios/experiment11FrontTail/{numFlows}flows/{scenario_name}.xml")'
                    w(f"*.scenarioManager.script = {scenario_doc}")
                    if alg == "satcp":
                        w(f"**.tcp.scenario = {scenario_doc}")
                    w()

    clone_raynet_ini_variants(out_dir / "experiment11FrontTail_bbr.ini")
