...
if __name__ == "__main__":
    ...
    experiment = "experiment11FrontTail"
    flowBatches = [5, 10, 20]
    clientsRtts = [50]
    runs = 5
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 1
        subprocess.Popen("python3 generateExperiment11FrontTailScenarios.py", shell=True).communicate(timeout=60)
        subprocess.Popen("python3 generateExperiment11FrontTailIniFile.py", shell=True).communicate(timeout=60)

        Path("experiment11FrontTailrunTimes.txt").unlink(missing_ok=True)
        for c in congControlList:
            subprocess.Popen("rm -r ../../paperExperiments/experiment11FrontTail/csvs/" + c, shell=True).communicate(timeout=30)
            
        with open('experiment11FrontTailrunTimes.txt', 'w') as f1:
            f1.write("--Experiment 11 Runtimes (s)--")
            simulation_configs = []
            for cc in congControlList:
                fileName = '../../paperExperiments/experiment11FrontTail/experiment11FrontTail_' + cc + '.ini'
                if not os.path.exists(fileName):
                    print("Missing ini file " + fileName + ", skipping...")
                    continue
                print("----------queueing experiment 11 " + cc + " simulations------------")
                iniName = "experiment11FrontTail_" + cc + ".ini"
                simulation_configs.extend(collect_simulation_configs(
                    cc, iniName, runList, "../../paperExperiments/experiment11FrontTail"
                ))
            run_simulation_configs(simulation_configs, "../../paperExperiments/experiment11FrontTail", cores, f1)
        time.sleep(5)
    currStep += 1
    currentProc = 0
    processList.clear()
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 2
        ...
        folderLoc =  '../../paperExperiments/experiment11FrontTail/results/'
        ...
        for file in os.listdir(folderLoc):
            if(file.endswith(".vec")):
                ...
                processList.append(subprocess.Popen("opp_scavetool export -o "+ "results/"+ file[:-7] + ".csv -F CSV-R " + "results/" + file , shell=True, cwd='../../paperExperiments/experiment11FrontTail/'))
        ...
        for protocol in congControlList:
            for flowBatch in flowBatches:
                for rtt in clientsRtts:
                    for run in runList:
                        filePath = '../../paperExperiments/experiment11FrontTail/results/'+ protocol_config_prefix(protocol) + '_' + str(flowBatch) + 'flows_' + str(rtt) + 'ms_Run' + str(run) + '.csv'
                        print(filePath)
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 4
        subprocess.Popen("mkdir ../../plots/experiment11FrontTail", shell=True).communicate(timeout=10) 
        subprocess.Popen("rm -r *", shell=True, cwd='../../plots/experiment11FrontTail').communicate(timeout=200) 
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 5
        ...
        subprocess.Popen("mkdir cumulative", shell=True, cwd='../../plots/experiment11FrontTail/').communicate(timeout=10)
        time.sleep(3)
        p = subprocess.Popen("python3 ../../../pythonScripts/runPlotVariantsFrontTail.py ../../../pythonScripts/experiment11FrontTail/plotScatterUtilDelay.py", shell=True, cwd='../../plots/experiment11FrontTail/cumulative')
        p.wait(timeout=3600)
        time.sleep(1)
    currStep += 1
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 6
        ...
        for protocol in congControlList:
            for flowBatch in flowBatches:
                ...
                for rtt in clientsRtts:
                    for run in runList:
                        dirPath = '../../plots/experiment11FrontTail/' + protocol + '/' + batchFolder + '/' + str(rtt) + 'ms' + '/run' + str(run) + '/'
                        ...
                        for goodputFile in goodputFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment11FrontTail/plotGoodput.py " + goodputFile[0], dirPath + goodputFile[1]))
                        
                        for throughputFile in throughputFileList:
                            if os.path.exists("../../../pythonScripts/experiment11FrontTail/plotThroughput.py"):
                                processListStr.append(("python3 ../../../../../../../pythonScripts/experiment11FrontTail/plotThroughput.py " + throughputFile[0], dirPath + throughputFile[1]))
                                
                        for cwndFile in cwndFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment11FrontTail/plotCwnd.py " + cwndFile[0], dirPath + cwndFile[1]))
                        
                        for queueLengthFile in queueLengthFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment11FrontTail/plotQueueLength.py " + queueLengthFile[0], dirPath + queueLengthFile[1]))
                        
                        for rttFile in rttFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment11FrontTail/plotRtt.py " + rttFile[0], dirPath + rttFile[1]))
                        
                        for inflightFile in inflightFileList:
                            if os.path.exists("../../../pythonScripts/experiment11FrontTail/plotBytesInFlight.py"):
                                processListStr.append(("python3 ../../../../../../../pythonScripts/experiment11FrontTail/plotBytesInFlight.py " + inflightFile[0], dirPath + inflightFile[1]))
                        
                        for aggrePlotFile in aggrCwndFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment11FrontTail/plotCwnd.py " + aggrePlotFile[0], dirPath + aggrePlotFile[1]))
                        
                        for aggreGpPlotFile in aggrGoodputFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experiment11FrontTail/plotGoodput.py " + aggreGpPlotFile[0], dirPath + aggreGpPlotFile[1]))
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 7
        ...
        merge_pdfs_in_folders("../../plots/experiment11FrontTail")
