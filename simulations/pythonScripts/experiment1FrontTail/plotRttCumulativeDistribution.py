...
if __name__ == "__main__":
    ...
    for protocol in protocols:
        for run in runs:
            filePath = (
                f"../../paperExperiments/experiment1FrontTail/csvs/{protocol}/run{run}/"
                "singledumbbell.client[0].tcp.conn/rtt.csv"
            )
            if os.path.exists(filePath):
                with open(f"../../paperExperiments/baseRtts/experiment1FrontTail/run{run}.json") as jsonData:
    ...
    if cdf_exports:
        export_plot_dataframe(
            ...
            metadata={
                "experiment": "experiment1FrontTail",
                "plot": "joined_rtt_cdf",
