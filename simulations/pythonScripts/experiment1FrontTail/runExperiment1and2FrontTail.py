...
LOG_ROOT = SIM_ROOT / "logs" / "experiment1and2FrontTail"
...
def plot_cumulative(script_name: str) -> None:
    cumulative_dir = PLOTS_ROOT / "experiment1and2FrontTailCumulative"
    cumulative_dir.mkdir(parents=True, exist_ok=True)
    run_checked(
        [
            sys.executable,
            str(SCRIPT_DIR.parent / "runPlotVariantsFrontTail.py"),
...
def plot_protocol_legends() -> None:
    ...
    run_checked(
        [
            sys.executable,
            str(SCRIPT_DIR.parent / "runPlotVariantsFrontTail.py"),
            str(SCRIPT_DIR.parent / "plotHeaderLinesFrontTail.py"),
        ],
        cwd=cumulative_dir,
...
def generate_inputs() -> None:
    run_checked([sys.executable, "generateExperiment1FrontTailScenarios.py"], SCRIPT_DIR, "Generating experiment 1 scenarios")
    run_checked([sys.executable, "generateExperiment1FrontTailIniFile.py"], SCRIPT_DIR, "Generating experiment 1 ini files")
    run_checked(
        [sys.executable, "../experiment2FrontTail/generateExperiment2FrontTailScenarios.py"],
        SCRIPT_DIR,
        "Generating experiment 2 scenarios",
    )
    run_checked(
        [sys.executable, "../experiment2FrontTail/generateExperiment2FrontTailIniFile.py"],
        SCRIPT_DIR,
...
def main() -> int:
    ...
    experiments = ["experiment1FrontTail", "experiment2FrontTail"]
    ...
    if step_enabled(curr_step, args):
        ...
        entries = collect_config_entries("experiment1FrontTail", protocols, run_list)
        run_simulations(entries, args)
    curr_step += 1

    if step_enabled(curr_step, args):
        if not entries:
            entries = collect_config_entries("experiment2FrontTail", protocols, run_list)
        else:
            entries = collect_config_entries("experiment2FrontTail", protocols, run_list)
        run_simulations(entries, args)
    curr_step += 1

    all_entries = collect_config_entries("experiment1FrontTail", protocols, run_list) + collect_config_entries(
        "experiment2FrontTail", protocols, run_list
    )

    if step_enabled(curr_step, args):
        export_csvs(collect_config_entries("experiment1FrontTail", protocols, run_list), args)
    curr_step += 1

    if step_enabled(curr_step, args):
        export_csvs(collect_config_entries("experiment2FrontTail", protocols, run_list), args)
    curr_step += 1
