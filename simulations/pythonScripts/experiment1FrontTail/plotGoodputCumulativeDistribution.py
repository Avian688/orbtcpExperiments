...
from plotDataExportFrontTail import build_cdf_points, export_plot_dataframe
from plotProtocolSupportFrontTail import EXPERIMENT_1_PROTOCOLS, PROTOCOL_COLORS, PROTOCOL_LABELS
...
if __name__ == "__main__":
    ...
    for protocol in protocols:
        for run in runs:
            filePath = f"../../paperExperiments/experiment1FrontTail/csvs/{protocol}/run{run}/singledumbbell.server[0].app[0]/goodput.csv"
            if os.path.exists(filePath):
                with open(f"../../paperExperiments/bandwidths/experiment1FrontTail/run{run}.json") as jsonData:
    ...
    for protocol in protocols:
        for run in runs:
            filePath2 = f"../../paperExperiments/experiment2FrontTail/csvs/{protocol}/run{run}/singledumbbell.server[0].app[0]/goodput.csv"
            if os.path.exists(filePath2):
                with open(f"../../paperExperiments/bandwidths/experiment2FrontTail/run{run}.json") as jsonData:
                    d = json.load(jsonData)
    ...
    if cdf_exports:
        export_plot_dataframe(
            ...
            metadata={
                "experiment": "experiment1FrontTail_and_2",
                "plot": "joined_goodput_cdf",
                "description": "Final CDF points plotted for average goodput; experiment2FrontTail is represented by bw-rtt-loss.",
            },
        )
    export_plot_dataframe(
