...
VECTOR_NAMES = ("cwnd", "rtt", "U", "tau", "queueingDelay", "goodput")
...
def main() -> None:
    if len(sys.argv) != 5:
        raise SystemExit(
            "Usage: extractSingleCsvFile.py <CSV-R file> <variant> <workload> <run>"
        )
    ...
    workload = sys.argv[3]
    run = int(sys.argv[4])
    out_root = Path("../../paperExperiments/experiment13FrontTail/csvs") / variant / workload / f"run{run}"
    ...
    for name in VECTOR_NAMES:
        ...
        for _, row in matching_vectors.iterrows():
            ...
            pd.DataFrame({"time": times[:sample_count], name: values[:sample_count]}).to_csv(
                out_dir / f"{name}.csv",
                index=False,
            )
