...
from experiment13FrontTailSupport import (
    FULL_ORBCC,
    MSS_BYTES,
    PINT_VARIANTS,
    QUEUE_PACKETS,
...
from raynetExperimentSupportFrontTail import common_ned_path
...
def write_general(f, pint: bool) -> None:
    ...
    w(common_ned_path())
    w()
    w("network = orbtcpexperiments.simulations.paperExperiments.experiment13FrontTail.singledumbbell")
    ...
    bottleneck_queue = "PintFrontTailQueue" if pint else "IntFrontTailQueue"
    ...
    w('**.ppp[*].queue.typename = "DropHeadQueue"')
...
def write_config(f, variant, workload: str, run: int) -> None:
    ...
    f.write(f'*.scenarioManager.script = xmldoc("../scenarios/experiment13FrontTail/{scenario_name(workload, run)}.xml")\n\n')
...
def main() -> None:
    out_dir = Path("../../paperExperiments/experiment13FrontTail")
