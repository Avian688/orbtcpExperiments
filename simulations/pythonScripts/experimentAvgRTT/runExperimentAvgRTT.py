...
if __name__ == "__main__":
    ...
    congControlList = ["orbtcp_pint_per_flow_rtt", "orbtcp_pint"]
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 1
        ...
        with open('experimentAvgRTTrunTimes.txt', 'w') as f1:
            ...
            with ThreadPoolExecutor(max_workers=max(1, cores)) as executor:
                futures = {
                    executor.submit(subprocess.run, command, shell=True, cwd='../../paperExperiments/experimentAvgRTT', stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL): configName
            ...
            if failures:
                raise RuntimeError("Experiment AvgRTT simulations failed: " + ", ".join(failures))
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 2
        currentProc = 0
        print("\nAll experiments in Experiment AvgRTT has been run!\n")
        folderLoc =  '../../paperExperiments/experimentAvgRTT/results/'
        print("------------ Generating CSV Files for experiment AvgRTT ------------")
        
        fileList = []
        for file in os.listdir(folderLoc):
            if(file.endswith(".vec")):
                f = os.path.join(folderLoc, file)
                processList.append(subprocess.Popen("opp_scavetool export -o "+ "results/"+ file[:-7] + ".csv -F CSV-R " + "results/" + file , shell=True, cwd='../../paperExperiments/experimentAvgRTT/'))
        ...
        processList.clear()
        currentProc = 0
    print("CSVs created for experiments AvgRTT!\n")
    currStep += 1
    
    if(currStep <= endStep and currStep >= startStep): #STEP 3
        currentProc = 0
        print("Extracting CSV data!!\n")
        print("------------ Extracting CSV Files for experiment AvgRTT------------")
        processListStr = []
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 4
        subprocess.Popen("mkdir ../../plots/experimentAvgRTT", shell=True).communicate(timeout=10) 
        subprocess.Popen("rm -r *", shell=True, cwd='../../plots/experimentAvgRTT').communicate(timeout=200) 
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 5
        ...
        subprocess.Popen("mkdir cumulative", shell=True, cwd='../../plots/experimentAvgRTT/').communicate(timeout=10)
        time.sleep(3)
        p = subprocess.Popen("python3 ../../../pythonScripts/experimentAvgRTT/plotGoodputRatio.py", shell=True, cwd='../../plots/experimentAvgRTT/cumulative')
        p.wait(timeout=3600)
        time.sleep(1)
    currStep += 1
            ...
            for buf in buffersizes:
                for rtt in clientsRtts:
                    for run in runList:
                        ...
                        dirPath = '../../plots/experimentAvgRTT/' + protocol + '/' + buf + '/' + str(rtt) + 'ms' + '/run' + str(run) + '/'
                        ...
                        for cwndFile in cwndFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experimentAvgRTT/plotCwnd.py " + cwndFile[0], dirPath + cwndFile[1]))
                        
                        for rttFile in rttFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experimentAvgRTT/plotRtt.py " + rttFile[0], dirPath + rttFile[1]))
                                
                        # for tauFile in tauFileList:
                        #     processListStr.append(("python3 ../../../../../../../pythonScripts/experimentAvgRTT//plotTau.py " + tauFile[0], dirPath + tauFile[1]))
                                
                        # for UFile in UFileList:
                        #     processListStr.append(("python3 ../../../../../../../pythonScripts/experimentAvgRTT/plotU.py " + UFile[0], dirPath + UFile[1]))
                                
                        for goodputFile in goodputFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experimentAvgRTT/plotGoodput.py " + goodputFile[0], dirPath + goodputFile[1]))
                                
                        for queueLengthFile in queueLengthFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experimentAvgRTT/plotQueueLength.py " + queueLengthFile[0], dirPath + queueLengthFile[1]))
                                
                        for aggrePlotFile in aggrPlotsFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experimentAvgRTT/plotCwnd.py " + aggrePlotFile[0], dirPath + aggrePlotFile[1]))
                            
                        for aggreGpPlotFile in aggrPlotsGoodputFileList:
                            processListStr.append(("python3 ../../../../../../../pythonScripts/experimentAvgRTT/plotGoodput.py " + aggreGpPlotFile[0], dirPath + aggreGpPlotFile[1]))
    ...
    if(currStep <= endStep and currStep >= startStep): #STEP 7
        ...
        merge_pdfs_in_folders("../../plots/experimentAvgRTT")
